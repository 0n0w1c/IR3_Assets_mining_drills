if not (
        mods["IndustrialRevolution3Assets1"]
...
local function generate_recycling_recipe(name)
    if not recycling then return end
...
generate_recycling_recipe("burner-mining-drill")
...
generate_recycling_recipe("electric-mining-drill")
...
if mods["space-age"] then
    ...
    generate_recycling_recipe("big-mining-drill")
else
    if mods["aai-industry"]
        and data.raw["mining-drill"]
        and data.raw["mining-drill"]["area-mining-drill"]
    then
        ...
        generate_recycling_recipe("area-mining-drill")
    end
end
